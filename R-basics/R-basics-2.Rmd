...
`seq_along()` is used for generating a sequence of elements the same as that of the length of the input vector. `cat()` converts its arguments to character strings, concatenates them, separating them by the string given to the `sep` parameter, and then prints them.
...
To call a function, we just put the function name and added the necessary arguments inside the parenthesis.
...
Let's start with a simple example. Pay special attention to the use of parenthesis and curly braces.
...
Now, let's turn back to the `show_files()` function and focus on the `for` loop in its function body.
...
In order to do a KWIC analysis, we will need to break up a text into tokens. In [R-basics-1](./R-basics-1.Rmd), we have learned that the `unnest()` function from the `tidytext` library can help us tokenize a text.

```{r}
### use unnest() to tokenize text
...
If we would like to grab the preceding and the following word of the target word, we could do so by iterating through the indices in the `position_w` vector and for each position, we get the neighboring words and concatenate them with the target word.

### Set a context window to get KWIC line

```{r}
for (pos in pos_w){ # for each position in position_w
  start = pos - 1
  end = pos + 1
...
for (i in seq_along(pos_dog)){# for each position in position_dog
  ...
  kwic_line <- paste((melville_words %>% slice(start:end))[['word']], collapse=' ')
...
sense_kwic <- function(f_loc){
  sense <- scan("./data/austen.txt", what='character', sep='\n') # read in the data
